...
HEADER_DIR="$PROJECT_DIR/Sources/cxx-ladybug-prebuilt/include"
...
if [ -f "$TARGET_DIR/$LIB_NAME" ] && [ -f "$HEADER_DIR/lbug.h" ]; then
...
fetch_release_archive() {
  ...
  if [ -n "$VERSION_OVERRIDE" ]; then
    ...
    version="$(curl -fsSL "https://api.github.com/repos/${REPOSITORY}/releases/latest" | sed -nE 's/.*"tag_name"[[:space:]]*:[[:space:]]*"v?([^"]+)".*/\1/p')"
  fi
  if [ -z "$version" ]; then
    echo "Failed to resolve latest release version for ${REPOSITORY}" >&2
    exit 1
  fi

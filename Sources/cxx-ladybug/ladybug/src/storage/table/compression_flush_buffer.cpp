...
ColumnChunkMetadata CompressedFlushBuffer::operator()(std::span<const uint8_t> buffer,
    ...
    const auto compressedBuffer = std::make_unique<uint8_t[]>(LADYBUG_PAGE_SIZE);
    auto numPages = 0u;
    const auto numValuesPerPage = metadata.compMeta.numValues(LADYBUG_PAGE_SIZE, dataType);
    ...
    while (valuesRemaining > 0) {
        const auto compressedSize = alg->compressNextPage(bufferStart, valuesRemaining,
            compressedBuffer.get(), LADYBUG_PAGE_SIZE, metadata.compMeta);
        ...
        if (compressedSize < LADYBUG_PAGE_SIZE) {
            memset(compressedBuffer.get() + compressedSize, 0, LADYBUG_PAGE_SIZE - compressedSize);
        }
    ...
    if (!dataFH->isInMemoryMode() && numPages < entry.numPages) {
        memset(compressedBuffer.get(), 0, LADYBUG_PAGE_SIZE);
...
std::pair<std::unique_ptr<uint8_t[]>, uint64_t> flushCompressedFloats(const CompressionAlg& alg,
    ...
    while (valuesRemaining > 0) {
        ...
        (void)castedAlg.compressNextPageWithExceptions(bufferCursor,
            ...
            LADYBUG_PAGE_SIZE, EncodeExceptionView<T>{exceptionBufferCursor},

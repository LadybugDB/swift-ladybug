...
public:
    StringColumnWriterState(ladybug_parquet::format::RowGroup& rowGroup, uint64_t colIdx,
...
public:
    ...
    inline ladybug_parquet::format::Encoding::type getEncoding(
        BasicColumnWriterState& writerState) override {
        auto& state = reinterpret_cast<StringColumnWriterState&>(writerState);
        return state.isDictionaryEncoded() ? ladybug_parquet::format::Encoding::RLE_DICTIONARY :
                                             ladybug_parquet::format::Encoding::PLAIN;
    }

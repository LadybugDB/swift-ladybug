...
public:
    BasicColumnWriterState(ladybug_parquet::format::RowGroup& rowGroup, uint64_t colIdx)
...
public:
    std::unique_ptr<ColumnWriterState> initializeWriteState(
        ladybug_parquet::format::RowGroup& rowGroup) override;
...
protected:
    ...
    virtual ladybug_parquet::format::Encoding::type getEncoding(BasicColumnWriterState& /*state*/) {
        return ladybug_parquet::format::Encoding::PLAIN;
    }
    ...
    void setParquetStatistics(BasicColumnWriterState& state,
        ladybug_parquet::format::ColumnChunk& column);
    void registerToRowGroup(ladybug_parquet::format::RowGroup& rowGroup);
};

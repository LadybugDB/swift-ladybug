...
struct ParquetReaderScanState {
    ...
    std::unique_ptr<ladybug_apache::thrift::protocol::TProtocol> thriftFileProto;
...
public:
    ...
    ladybug_parquet::format::FileMetaData* getMetadata() const { return metadata.get(); }

private:
    std::unique_ptr<ladybug_apache::thrift::protocol::TProtocol> createThriftProtocol(
        common::FileInfo* fileInfo_, bool prefetch_mode) {
        return std::make_unique<
            ladybug_apache::thrift::protocol::TCompactProtocolT<ThriftFileTransport>>(
            std::make_shared<ThriftFileTransport>(fileInfo_, prefetch_mode));
    }
    const ladybug_parquet::format::RowGroup& getGroup(ParquetReaderScanState& state) {
        KU_ASSERT(
    ...
    static common::LogicalType deriveLogicalType(const ladybug_parquet::format::SchemaElement& s_ele);
    void initMetadata();
...
private:
    ...
    std::unique_ptr<ladybug_parquet::format::FileMetaData> metadata;
    main::ClientContext* context;
};

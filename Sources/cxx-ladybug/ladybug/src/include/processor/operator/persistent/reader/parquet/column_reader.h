...
public:
    ColumnReader(ParquetReader& reader, common::LogicalType type,
        const ladybug_parquet::format::SchemaElement& schema, common::idx_t fileIdx,
    ...
    static std::unique_ptr<ColumnReader> createReader(ParquetReader& reader,
        common::LogicalType type, const ladybug_parquet::format::SchemaElement& schema,
    ...
    void decompressInternal(ladybug_parquet::format::CompressionCodec::type codec, const uint8_t* src,
        uint64_t srcSize, uint8_t* dst, uint64_t dstSize);
    void preparePageV2(ladybug_parquet::format::PageHeader& pageHdr);
    void preparePage(ladybug_parquet::format::PageHeader& pageHdr);
    void prepareDataPage(ladybug_parquet::format::PageHeader& pageHdr);
...
protected:
    const ladybug_parquet::format::SchemaElement& schema;
    ...
    const ladybug_parquet::format::ColumnChunk* chunk = nullptr;

...
inline void Hash::operation(const std::string_view& key, common::hash_t& result) {
    ...
    for (size_t i = 0u; i < key.size() / 8; i++) {
        auto blockHash = ladybug::function::murmurhash64(*(data64 + i));
        hashValue = ladybug::function::combineHashScalar(hashValue, blockHash);
    }
    uint64_t last = 0;
    for (size_t i = 0u; i < key.size() % 8; i++) {
        last |= static_cast<uint64_t>(key[key.size() / 8 * 8 + i]) << i * 8;
    }
    hashValue = ladybug::function::combineHashScalar(hashValue, ladybug::function::murmurhash64(last));
    result = hashValue;
}

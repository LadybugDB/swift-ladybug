...
public:
    ...
    LADYBUG_API static dtime_t fromCString(const char* buf, uint64_t len);
    LADYBUG_API static bool tryConvertInterval(const char* buf, uint64_t len, uint64_t& pos,
        dtime_t& result);
    LADYBUG_API static bool tryConvertTime(const char* buf, uint64_t len, uint64_t& pos,
        dtime_t& result);

    // Convert a time object to a string in the format "hh:mm:ss"
    LADYBUG_API static std::string toString(dtime_t time);

    LADYBUG_API static dtime_t fromTime(int32_t hour, int32_t minute, int32_t second,
        int32_t microseconds = 0);

    // Extract the time from a given timestamp object
    LADYBUG_API static void convert(dtime_t time, int32_t& out_hour, int32_t& out_min,
        int32_t& out_sec, int32_t& out_micros);

    LADYBUG_API static bool isValid(int32_t hour, int32_t minute, int32_t second,

...
struct LADYBUG_API internalID_t {
    offset_t offset;
    table_id_t tableID;
...
class LogicalType {
    ...
    LADYBUG_API LogicalType(const LogicalType& other);

public:
    LADYBUG_API LogicalType() : typeID{LogicalTypeID::ANY}, extraTypeInfo{nullptr} {
        physicalType = getPhysicalType(this->typeID);
    };
    explicit LADYBUG_API LogicalType(LogicalTypeID typeID, TypeCategory info = TypeCategory::INTERNAL);
    EXPLICIT_COPY_DEFAULT_MOVE(LogicalType);

    LADYBUG_API bool operator==(const LogicalType& other) const;
    LADYBUG_API bool operator!=(const LogicalType& other) const;
    ...
    LADYBUG_API LogicalTypeID getLogicalTypeID() const { return typeID; }
    bool containsAny() const;
    bool isInternalType() const { return category == TypeCategory::INTERNAL; }

    LADYBUG_API PhysicalTypeID getPhysicalType() const { return physicalType; }
    LADYBUG_API static PhysicalTypeID getPhysicalType(LogicalTypeID logicalType,
    ...
    LADYBUG_API static std::vector<LogicalType> copy(const std::vector<LogicalType>& types);
    LADYBUG_API static std::vector<LogicalType> copy(const std::vector<LogicalType*>& types);
    ...
    static LADYBUG_API LogicalType DECIMAL(uint32_t precision, uint32_t scale);
    ...
    static LADYBUG_API LogicalType STRUCT(std::vector<StructField>&& fields);

    static LADYBUG_API LogicalType RECURSIVE_REL(std::vector<StructField>&& fields);

    static LADYBUG_API LogicalType NODE(std::vector<StructField>&& fields);

    static LADYBUG_API LogicalType REL(std::vector<StructField>&& fields);

    static LADYBUG_API LogicalType UNION(std::vector<StructField>&& fields);

    static LADYBUG_API LogicalType LIST(LogicalType childType);
    template<class T>
    static inline LogicalType LIST(T&& childType) {
        return LogicalType::LIST(LogicalType(std::forward<T>(childType)));
    }

    static LADYBUG_API LogicalType MAP(LogicalType keyType, LogicalType valueType);
    template<class T>
    ...
    static LADYBUG_API LogicalType ARRAY(LogicalType childType, uint64_t numElements);
    template<class T>
...
private:
    ...
    friend class ladybug::processor::ParquetReader;
...
class LADYBUG_API ExtraTypeInfo {
public:
...
class LADYBUG_API UDTTypeInfo : public ExtraTypeInfo {
public:
...
class LADYBUG_API ListTypeInfo : public ExtraTypeInfo {
public:
...
class LADYBUG_API ArrayTypeInfo final : public ListTypeInfo {
public:
...
struct LADYBUG_API DecimalType {
...
struct LADYBUG_API LogicalTypeUtils {

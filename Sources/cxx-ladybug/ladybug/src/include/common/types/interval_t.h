...
enum class LADYBUG_API DatePartSpecifier : uint8_t {
    YEAR,
    MONTH,
    DAY,
    DECADE,
...
public:
    ...
    LADYBUG_API static void addition(interval_t& result, uint64_t number, std::string specifierStr);
    LADYBUG_API static interval_t fromCString(const char* str, uint64_t len);
    LADYBUG_API static std::string toString(interval_t interval);
    LADYBUG_API static bool greaterThan(const interval_t& left, const interval_t& right);
    LADYBUG_API static void normalizeIntervalEntries(interval_t input, int64_t& months, int64_t& days,
        int64_t& micros);
    LADYBUG_API static void tryGetDatePartSpecifier(std::string specifier, DatePartSpecifier& result);
    LADYBUG_API static int32_t getIntervalPart(DatePartSpecifier specifier, interval_t timestamp);
    LADYBUG_API static int64_t getMicro(const interval_t& val);
    LADYBUG_API static int64_t getNanoseconds(const interval_t& val);
    LADYBUG_API static const regex::RE2& regexPattern1();
    LADYBUG_API static const regex::RE2& regexPattern2();
};

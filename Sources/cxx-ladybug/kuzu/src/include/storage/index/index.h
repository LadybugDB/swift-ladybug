...
enum class LADYBUG_API IndexConstraintType : uint8_t {
    PRIMARY = 0,              // Primary key index
    SECONDARY_NON_UNIQUE = 1, // Secondary index that is not unique
};

enum class LADYBUG_API IndexDefinitionType : uint8_t {
    BUILTIN = 0,
    EXTENSION = 1,
};
...
struct LADYBUG_API IndexStorageInfo {
    IndexStorageInfo() {}
...
public:
    ...
    void serialize(common::Serializer& ser) const;
    LADYBUG_API void load(main::ClientContext* context, StorageManager* storageManager);

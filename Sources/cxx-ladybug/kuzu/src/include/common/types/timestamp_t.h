...
struct LADYBUG_API timestamp_t {
    int64_t value = 0;
...
class Timestamp {
public:
    LADYBUG_API static timestamp_t fromCString(const char* str, uint64_t len);

    // Convert a timestamp object to a std::string in the format "YYYY-MM-DD hh:mm:ss".
    LADYBUG_API static std::string toString(timestamp_t timestamp);

    // Date header is in the format: %Y%m%d.
    LADYBUG_API static std::string getDateHeader(const timestamp_t& timestamp);

    // Timestamp header is in the format: %Y%m%dT%H%M%SZ.
    LADYBUG_API static std::string getDateTimeHeader(const timestamp_t& timestamp);

    LADYBUG_API static date_t getDate(timestamp_t timestamp);

    LADYBUG_API static dtime_t getTime(timestamp_t timestamp);

    // Create a Timestamp object from a specified (date, time) combination.
    LADYBUG_API static timestamp_t fromDateTime(date_t date, dtime_t time);

    LADYBUG_API static bool tryConvertTimestamp(const char* str, uint64_t len, timestamp_t& result);

    // Extract the date and time from a given timestamp object.
    LADYBUG_API static void convert(timestamp_t timestamp, date_t& out_date, dtime_t& out_time);

    // Create a Timestamp object from the specified epochMs.
    LADYBUG_API static timestamp_t fromEpochMicroSeconds(int64_t epochMs);

    // Create a Timestamp object from the specified epochMs.
    LADYBUG_API static timestamp_t fromEpochMilliSeconds(int64_t ms);

    // Create a Timestamp object from the specified epochSec.
    LADYBUG_API static timestamp_t fromEpochSeconds(int64_t sec);

    // Create a Timestamp object from the specified epochNs.
    LADYBUG_API static timestamp_t fromEpochNanoSeconds(int64_t ns);

    LADYBUG_API static int32_t getTimestampPart(DatePartSpecifier specifier, timestamp_t timestamp);

    LADYBUG_API static timestamp_t trunc(DatePartSpecifier specifier, timestamp_t date);

    LADYBUG_API static int64_t getEpochNanoSeconds(const timestamp_t& timestamp);

    LADYBUG_API static int64_t getEpochMilliSeconds(const timestamp_t& timestamp);

    LADYBUG_API static int64_t getEpochSeconds(const timestamp_t& timestamp);

    LADYBUG_API static bool tryParseUTCOffset(const char* str, uint64_t& pos, uint64_t len,
    ...
    LADYBUG_API static timestamp_t getCurrentTimestamp();
};

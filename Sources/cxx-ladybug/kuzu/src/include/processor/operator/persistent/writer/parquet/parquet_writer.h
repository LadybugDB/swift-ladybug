...
class ParquetWriterTransport : public ladybug_apache::thrift::protocol::TTransport {
public:
...
struct PreparedRowGroup {
    ladybug_parquet::format::RowGroup rowGroup;
...
public:
    ParquetWriter(std::string fileName, std::vector<common::LogicalType> types,
        std::vector<std::string> names, ladybug_parquet::format::CompressionCodec::type codec,
    ...
    inline ladybug_parquet::format::CompressionCodec::type getCodec() { return codec; }
    inline ladybug_apache::thrift::protocol::TProtocol* getProtocol() { return protocol.get(); }
    inline ladybug_parquet::format::Type::type getParquetType(uint64_t schemaIdx) {
        return fileMetaData.schema[schemaIdx].type;
    }
    void flush(FactorizedTable& ft);
    void finalize();
    static ladybug_parquet::format::Type::type convertToParquetType(const common::LogicalType& type);
    static void setSchemaProperties(const common::LogicalType& type,
        ladybug_parquet::format::SchemaElement& schemaElement);
...
private:
    ...
    ladybug_parquet::format::CompressionCodec::type codec;
    std::unique_ptr<common::FileInfo> fileInfo;
    std::shared_ptr<ladybug_apache::thrift::protocol::TProtocol> protocol;
    ladybug_parquet::format::FileMetaData fileMetaData;
    std::mutex lock;

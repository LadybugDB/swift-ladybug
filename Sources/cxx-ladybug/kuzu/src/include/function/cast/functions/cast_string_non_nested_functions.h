...
LogicalType LADYBUG_API inferMinimalTypeFromString(const std::string& str);
LogicalType LADYBUG_API inferMinimalTypeFromString(std::string_view str);
...
void LADYBUG_API castStringToBool(const char* input, uint64_t len, bool& result);
...
LADYBUG_API inline bool trySimpleIntegerCast(const char* input, uint64_t len, T& result) {
...
LADYBUG_API inline void simpleIntegerCast(const char* input, uint64_t len, T& result,
...
inline bool tryDoubleCast(const char* input, uint64_t len, T& result) {
    ...
    auto end = input + len;
    auto parse_result = ladybug_fast_float::from_chars(input, end, result);

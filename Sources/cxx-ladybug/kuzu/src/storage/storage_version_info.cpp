...
storage_version_t StorageVersionInfo::getStorageVersion() {
    ...
    if (!storageVersionInfo.contains(LADYBUG_CMAKE_VERSION)) {
        // If the current LADYBUG_CMAKE_VERSION is not in the map,
        // then we must run the newest version of ladybug
        // LCOV_EXCL_START
    ...
    return storageVersionInfo.at(LADYBUG_CMAKE_VERSION);
}

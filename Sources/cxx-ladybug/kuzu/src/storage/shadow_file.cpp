...
void ShadowFile::applyShadowPages(ClientContext& context) const {
    const auto pageBuffer = std::make_unique<uint8_t[]>(LADYBUG_PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        ...
        dataFileInfo->writeFile(pageBuffer.get(), LADYBUG_PAGE_SIZE,
            record.originalPageIdx * LADYBUG_PAGE_SIZE);
...
void ShadowFile::replayShadowPageRecords(ClientContext& context) {
    ...
    const auto headerBuffer = std::make_unique<uint8_t[]>(LADYBUG_PAGE_SIZE);
    shadowFileInfo->readFromFile(headerBuffer.get(), LADYBUG_PAGE_SIZE, 0);
    ...
    reader->resetReadOffset((header.numShadowPages + 1) * LADYBUG_PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        shadowFileInfo->readFromFile(pageBuffer.get(), LADYBUG_PAGE_SIZE,
            shadowPageIdx * LADYBUG_PAGE_SIZE);
        dataFileInfo->writeFile(pageBuffer.get(), LADYBUG_PAGE_SIZE,
            record.originalPageIdx * LADYBUG_PAGE_SIZE);
        shadowPageIdx++;
    }
...
void ShadowFile::flushAll() const {
    ...
    writer->setFileOffset(shadowingFH->getNumPages() * LADYBUG_PAGE_SIZE);

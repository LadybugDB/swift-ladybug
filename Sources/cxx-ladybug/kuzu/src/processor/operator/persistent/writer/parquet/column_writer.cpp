...
using namespace ladybug_parquet::format;
...
std::unique_ptr<ColumnWriter> ColumnWriter::createWriterRecursive(
    std::vector<ladybug_parquet::format::SchemaElement>& schemas, ParquetWriter& writer,
    ...
    case LogicalTypeID::STRUCT: {
        ...
        ladybug_parquet::format::SchemaElement schema_element;
    ...
    case LogicalTypeID::LIST: {
        ...
        ladybug_parquet::format::SchemaElement optionalElem;
        ...
        ladybug_parquet::format::SchemaElement repeatedElem;
    ...
    case LogicalTypeID::MAP: {
        ...
        // 	}
        // }
        ladybug_parquet::format::SchemaElement topElement;
        ...
        // key_value element
        ladybug_parquet::format::SchemaElement kv_element;
...
void ColumnWriter::compressPage(common::BufferWriter& bufferedSerializer, size_t& compressedSize,
    ...
    case CompressionCodec::SNAPPY: {
        compressedSize = ladybug_snappy::MaxCompressedLength(bufferedSerializer.getSize());
        compressedBuf = std::unique_ptr<uint8_t[]>(new uint8_t[compressedSize]);
        ladybug_snappy::RawCompress(reinterpret_cast<const char*>(bufferedSerializer.getBlobData()),
            bufferedSerializer.getSize(), reinterpret_cast<char*>(compressedBuf.get()),
            &compressedSize);
        compressedData = compressedBuf.get();
        KU_ASSERT(compressedSize <= ladybug_snappy::MaxCompressedLength(bufferedSerializer.getSize()));
    } break;
    case CompressionCodec::ZSTD: {
        compressedSize = ladybug_zstd::ZSTD_compressBound(bufferedSerializer.getSize());
        compressedBuf = std::unique_ptr<uint8_t[]>(new uint8_t[compressedSize]);
        compressedSize = ladybug_zstd::ZSTD_compress((void*)compressedBuf.get(), compressedSize,
    ...
    case CompressionCodec::LZ4_RAW: {
        compressedSize = ladybug_lz4::LZ4_compressBound(bufferedSerializer.getSize());
        compressedBuf = std::unique_ptr<uint8_t[]>(new uint8_t[compressedSize]);
        compressedSize = ladybug_lz4::LZ4_compress_default(
            reinterpret_cast<const char*>(bufferedSerializer.getBlobData()),

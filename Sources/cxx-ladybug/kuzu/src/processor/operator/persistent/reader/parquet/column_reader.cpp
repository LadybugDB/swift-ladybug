...
using ladybug_parquet::format::CompressionCodec;
using ladybug_parquet::format::ConvertedType;
using ladybug_parquet::format::Encoding;
using ladybug_parquet::format::PageType;
using ladybug_parquet::format::Type;

ColumnReader::ColumnReader(ParquetReader& reader, LogicalType type,
    const ladybug_parquet::format::SchemaElement& schema, idx_t fileIdx, uint64_t maxDefinition,
    uint64_t maxRepeat)
...
void ColumnReader::initializeRead(uint64_t /*rowGroupIdx*/,
    const std::vector<ladybug_parquet::format::ColumnChunk>& columns,
    ladybug_apache::thrift::protocol::TProtocol& protocol) {
...
std::unique_ptr<ColumnReader> ColumnReader::createReader(ParquetReader& reader,
    common::LogicalType type, const ladybug_parquet::format::SchemaElement& schema, uint64_t fileIdx,
...
void ColumnReader::prepareRead(parquet_filter_t& /*filter*/) {
    ...
    defineDecoder.reset();
    block.reset();
    ladybug_parquet::format::PageHeader pageHdr;
...
void ColumnReader::decompressInternal(ladybug_parquet::format::CompressionCodec::type codec,
    ...
    case CompressionCodec::SNAPPY: {
        {
            ...
            auto res = ladybug_snappy::GetUncompressedLength(reinterpret_cast<const char*>(src),
        ...
        auto res = ladybug_snappy::RawUncompress(reinterpret_cast<const char*>(src), srcSize,
    ...
    case CompressionCodec::ZSTD: {
        auto res = ladybug_zstd::ZSTD_decompress(dst, dstSize, src, srcSize);
        // LCOV_EXCL_START
        if (ladybug_zstd::ZSTD_isError(res) || res != (size_t)dstSize) {
    ...
    case CompressionCodec::LZ4_RAW: {
        auto res = ladybug_lz4::LZ4_decompress_safe(reinterpret_cast<const char*>(src),
...
void ColumnReader::preparePageV2(ladybug_parquet::format::PageHeader& pageHdr) {
...
void ColumnReader::preparePage(ladybug_parquet::format::PageHeader& pageHdr) {
    auto& trans = reinterpret_cast<ThriftFileTransport&>(*protocol->getTransport());
...
void ColumnReader::prepareDataPage(ladybug_parquet::format::PageHeader& pageHdr) {

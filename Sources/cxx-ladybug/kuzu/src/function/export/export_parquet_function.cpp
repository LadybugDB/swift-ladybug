...
struct ParquetOptions {
    ladybug_parquet::format::CompressionCodec::type codec =
        ladybug_parquet::format::CompressionCodec::SNAPPY;
    ...
    void setCompression(common::Value& value) {
        ...
        if (strVal == "UNCOMPRESSED") {
            codec = ladybug_parquet::format::CompressionCodec::UNCOMPRESSED;
        } else if (strVal == "SNAPPY") {
            codec = ladybug_parquet::format::CompressionCodec::SNAPPY;
        } else if (strVal == "ZSTD") {
            codec = ladybug_parquet::format::CompressionCodec::ZSTD;
        } else if (strVal == "GZIP") {
            codec = ladybug_parquet::format::CompressionCodec::GZIP;
        } else if (strVal == "LZ4_RAW") {
            codec = ladybug_parquet::format::CompressionCodec::LZ4_RAW;
        } else {

...
int64_t CompressedFileSystem::readFile(ladybug::common::FileInfo& fileInfo, void* buf,
    size_t numBytes) const {
    auto& compressedFileInfo = fileInfo.cast<CompressedFileInfo>();
    return compressedFileInfo.readData(buf, numBytes);
}

void CompressedFileSystem::reset(ladybug::common::FileInfo& fileInfo) {
    auto& compressedFileInfo = fileInfo.cast<CompressedFileInfo>();
    compressedFileInfo.childFileInfo->reset();
    compressedFileInfo.initialize();
}

uint64_t CompressedFileSystem::getFileSize(const ladybug::common::FileInfo& fileInfo) const {
    auto& compressedFileInfo = fileInfo.constCast<CompressedFileInfo>();
    return compressedFileInfo.childFileInfo->getFileSize();
}

void CompressedFileSystem::syncFile(const ladybug::common::FileInfo& fileInfo) const {
    auto& compressedFileInfo = fileInfo.constCast<CompressedFileInfo>();

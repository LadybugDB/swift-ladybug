...
int RE2::Set::Add(const StringPiece& pattern, std::string* error) {
    ...
    RegexpStatus status;
    ladybug::regex::Regexp* re = Regexp::Parse(pattern, pf, &status);
    if (re == NULL) {
        if (error != NULL)
            *error = status.Text();
    ...
    ladybug::regex::Regexp* m = ladybug::regex::Regexp::HaveMatch(n, pf);
    if (re->op() == kRegexpConcat) {
        int nsub = re->nsub();
        PODArray<ladybug::regex::Regexp*> sub(nsub + 1);
        for (int i = 0; i < nsub; i++)
            sub[i] = re->sub()[i]->Incref();
        sub[nsub] = m;
        re->Decref();
        re = ladybug::regex::Regexp::Concat(sub.data(), nsub + 1, pf);
    } else {
        ladybug::regex::Regexp* sub[2];
        sub[0] = re;
        sub[1] = m;
        re = ladybug::regex::Regexp::Concat(sub, 2, pf);
    }
...
bool RE2::Set::Compile() {
    ...
    PODArray<ladybug::regex::Regexp*> sub(size_);
    ...
    Regexp::ParseFlags pf = static_cast<Regexp::ParseFlags>(options_.ParseFlags());
    ladybug::regex::Regexp* re = ladybug::regex::Regexp::Alternate(sub.data(), size_, pf);
